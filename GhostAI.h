...
private:
	...
	bool GetPath(class PathNode* startNode, class PathNode* targetNode);

	float getMinDistance(class PathNode* startNode, class PathNode* targetNode);
	std::vector<class PathNode*> formPath(std::unordered_map<class PathNode*, class PathNode*> p, class PathNode* c);
	class PathNode* getNearestNode(class PathNode* n);
	class PathNode* GetRandomChild(class PathNode* n);

	const float SCATTER_SPEED = 90.0f;
	const float FRIGHTENED_SPEED = 65.0f;
	const float DEAD_SPEED = 125.0f;
	int mCurrentNodeInPath = 0;
	Vector2 mLeft = Vector2(-1, 0);
	Vector2 mRight = Vector2(1, 0);
	Vector2 mUp = Vector2(0, -1);
	Vector2 mDown = Vector2(0, 1);
	
	float mTimeSinceScatter = 0.0f;
	float mTimeSinceFright = 0.0f;

	class PathNode* getTarget(class Ghost* g);
	bool mFirstIter = true;
	bool mFirstFright = true;

	void adjustPos(Vector2 v);
